 ...
 */
router.get('/', searchRunnersValidation, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { lat, lng, radius_km = '10', tags, limit = '20', offset = '0' } = req.query as {
      lat?: string;
      lng?: string;
      radius_km?: string;
      tags?: string | string[];
    ...
    res.status(500).json({ error: 'Failed to search runners', message: (error as Error).message });
  }
});
 ...
 */
router.get('/:id', getRunnerValidation, async (req: Request, res: Response): Promise<void> => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      res.status(400).json({ errors: errors.array() });
    ...
    res.status(500).json({ error: 'Failed to fetch runner', message: (error as Error).message });
  }
});
 ...
 */
router.post('/', authenticate, authorize('runner'), createRunnerValidation, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      res.status(400).json({ errors: errors.array() });
    ...
    if (existing.rows.length > 0) {
      res.status(409).json({ error: 'Runner profile already exists' });
      return;
    }
    ...
    res.status(500).json({ error: 'Failed to create runner profile', message: (error as Error).message });
  }
});
 ...
 */
router.patch('/:id', authenticate, authorize('runner'), updateRunnerValidation, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      res.status(400).json({ errors: errors.array() });
    ...
    res.status(500).json({ error: 'Failed to update runner profile', message: (error as Error).message });
  }
});
